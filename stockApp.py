...
loan_amount = st.sidebar.number_input(label='Enter the loan amount to be borrowed')
start_date = st.sidebar.date_input(label = 'Selection income start projection date1', value =None, min_value = date.today())
ZB_to_compare = st.sidebar.multiselect( 'Select the Zinsbindung scenarios you would like to compare?', ZB_array )
BSV_to_compare = st.sidebar.radio( 'Select wheather you wish to use BSV?', BSV_ind_array )
